...
pub struct CustomDrawApp {
    g: Graph<NodeData, (), Directed, DefaultIx, NodeShapeAnimated>,
}
...
fn generate_graph() -> StableGraph<NodeData, ()> {
    ...
    let a = g.add_node(NodeData { clockwise: true });
    let b = g.add_node(NodeData { clockwise: false });
    let c = g.add_node(NodeData { clockwise: false });
