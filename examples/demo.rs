...
use egui::{self, CollapsingHeader, Pos2, ScrollArea, Ui, Align2};
...
mod settings_local {
	...
	impl Default for SettingsNavigation {
		fn default() -> Self {
			Self { fit_to_screen_enabled: true, zoom_and_pan_enabled: false, zoom_speed: 0.1 }
		}
	}
	...
	impl Default for SettingsGraph {
		fn default() -> Self { Self { count_node: 25, count_edge: 50 } }
	}
}
...
mod drawers {
	...
	pub struct GraphCountSliders { pub nodes: usize, pub edges: usize }
	pub fn graph_count_sliders(ui: &mut Ui, mut v: GraphCountSliders, mut on_change: impl FnMut(i32, i32)) {
		...
		ui.horizontal(|ui| {
			...
			if ui.small_button("+1").clicked() { v.nodes = (v.nodes + 1).min(500); }
			if ui.small_button("+10").clicked() { v.nodes = (v.nodes + 10).min(500); }
		...
		ui.horizontal(|ui| {
			...
			if ui.small_button("+1").clicked() { v.edges = (v.edges + 1).min(500); }
			if ui.small_button("+10").clicked() { v.edges = (v.edges + 10).min(500); }
			delta_edges = v.edges as i32 - start as i32;
		});

		if delta_nodes != 0 || delta_edges != 0 { on_change(delta_nodes, delta_edges); }
	}
}
...
impl DemoApp {
	...
	fn remove_random_node(&mut self) { if let Some(i) = self.random_node_idx() { self.remove_node(i); } }
	fn remove_node(&mut self, idx: NodeIndex) { self.g.remove_node(idx); self.settings_graph.count_edge = self.g.edge_count(); }
	fn add_random_edge(&mut self) { if let (Some(a), Some(b)) = (self.random_node_idx(), self.random_node_idx()) { self.add_edge(a, b); } }
	fn add_edge(&mut self, a: NodeIndex, b: NodeIndex) { self.g.add_edge(a, b, ()); }
	fn remove_random_edge(&mut self) { if let Some(eidx) = self.random_edge_idx() { if let Some((a,b)) = self.g.edge_endpoints(eidx) { self.remove_edge(a, b); } } }
	fn remove_edge(&mut self, a: NodeIndex, b: NodeIndex) {
		...
		if let Some(edge_id) = edge_id_opt { self.g.remove_edge(edge_id); }
	}
	...
	fn handle_events(&mut self) {
		self.event_consumer.try_iter().for_each(|e| {
			if self.last_events.len() > EVENTS_LIMIT { self.last_events.remove(0); }
	...
	drawers::graph_count_sliders(
			ui,
			drawers::GraphCountSliders { nodes: self.settings_graph.count_node, edges: self.settings_graph.count_edge },
			|dn, de| {
				self.settings_graph.count_node = (self.settings_graph.count_node as i32 + dn).max(1) as usize;
				self.settings_graph.count_edge = (self.settings_graph.count_edge as i32 + de).max(0) as usize;
				if dn > 0 { for _ in 0..dn { self.add_random_node(); } } else if dn < 0 { for _ in 0..(-dn) { self.remove_random_node(); } }
				if de > 0 { for _ in 0..de { self.add_random_edge(); } } else if de < 0 { for _ in 0..(-de) { self.remove_random_edge(); } }
			},
		);
	}
	...
	fn ui_navigation(&mut self, ui: &mut Ui) {
		CollapsingHeader::new("Navigation").default_open(true).show(ui, |ui| {
			if ui.checkbox(&mut self.settings_navigation.fit_to_screen_enabled, "fit_to_screen").clicked() && self.settings_navigation.fit_to_screen_enabled {
	...
	fn ui_interaction(&mut self, ui: &mut Ui) {
		CollapsingHeader::new("Interaction").default_open(true).show(ui, |ui| {
			if ui.checkbox(&mut self.settings_interaction.dragging_enabled, "dragging").clicked() && self.settings_interaction.dragging_enabled {
			...
			ui.add_enabled_ui(!self.settings_interaction.node_selection_multi_enabled, |ui| {
				if ui.checkbox(&mut self.settings_interaction.node_selection_enabled, "node_selection").clicked() && self.settings_interaction.node_selection_enabled {
					self.settings_interaction.node_clicking_enabled = true;
				}
			});
			if ui.checkbox(&mut self.settings_interaction.node_selection_multi_enabled, "node_selection_multi").changed() && self.settings_interaction.node_selection_multi_enabled {
			...
			ui.add_enabled_ui(!self.settings_interaction.edge_selection_multi_enabled, |ui| {
				if ui.checkbox(&mut self.settings_interaction.edge_selection_enabled, "edge_selection").clicked() && self.settings_interaction.edge_selection_enabled {
					self.settings_interaction.edge_clicking_enabled = true;
				}
			});
			if ui.checkbox(&mut self.settings_interaction.edge_selection_multi_enabled, "edge_selection_multi").changed() && self.settings_interaction.edge_selection_multi_enabled {
	...
	fn ui_style(&mut self, ui: &mut Ui) { CollapsingHeader::new("Style").show(ui, |ui| { ui.checkbox(&mut self.settings_style.labels_always, "labels_always"); }); }
	...
	fn ui_events(&mut self, ui: &mut Ui) {
		CollapsingHeader::new("Last Events").default_open(true).show(ui, |ui| {
			#[cfg(feature = "events")]
			{
				if ui.button("clear").clicked() { self.last_events.clear(); }
				ScrollArea::vertical().max_height(200.0).show(ui, |ui| {
					for ev in self.last_events.iter().rev() { ui.label(ev); }
				});
			}
			#[cfg(not(feature = "events"))]
	...
	fn overlay_debug(&self, ctx: &egui::Context) {
		use egui::{Area, Frame, RichText, Color32};
		let text = {
			...
			format!("{fps}\n{n}\n{e}\n{z}\n{p}", fps=fps_line, n=n_line, e=e_line, z=zoom_line, p=pan_line)
		};
			...
			.show(ctx, |ui| {
				Frame::new()
					...
					.show(ui, |ui| {
						...
						ui.add_space(2.0);
						ui.label(RichText::new(text).monospace().color(Color32::WHITE).small());
					});
			});
	}
...
impl App for DemoApp {
	fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
		egui::SidePanel::right("right").default_width(260.0).show(ctx, |ui| {
		...
		egui::CentralPanel::default().show(ctx, |ui| {
			...
			let settings_style = &egui_graphs::SettingsStyle::new().with_labels_always(self.settings_style.labels_always);

			let mut view = egui_graphs::GraphView::<_,_,_,_,_,_,LayoutStateForceDirected,LayoutForceDirected>::new(&mut self.g)
...
fn main() { run_native("egui_graphs demo", eframe::NativeOptions::default(), Box::new(|cc| Ok(Box::new(DemoApp::new(cc))))).unwrap(); }
